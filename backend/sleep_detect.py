...
facial_landmarks_predictor = "C:/Users/lt.tung/Documents/daydianhtaioi/backend/shape_predictor_68_face_landmarks.dat"
predictor = dlib.shape_predictor(facial_landmarks_predictor)

# Load the model predictors for the eye condition
model = load_model('C:/Users/lt.tung/Documents/daydianhtaioi/backend/weights.149-0.01.hdf5')
...
while (True):
    ...
    if nofacetimeout == 30: # 30 frames of sleep time before we play an airhorn sound and write the file to a local repository
        playsound("C:/Users/lt.tung/Documents/daydianhtaioi/backend/veryloudairhorn.mp3", block = False)
        videoWriter.release()
        now = time.asctime( time.localtime(time.time()))
        x = str(now).replace(" ", "_") 
        x = x.replace(":",".")
        ...
        buffer = 0
#        firebase.convert("detected/"+f1, "detected/out")
